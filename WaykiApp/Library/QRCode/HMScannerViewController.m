...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    [HMScanner scaneImage:image completion:^(NSArray *values) {
        ...
        if (values.count > 0) {
            ...
            NSString *errorMsg = [LocalLanague dPLocalizedString:@"无法扫描到二维码，请重试"];
...
- (void)prepareNavigationBar {
    ...
    // 2> 标题
    self.title = NSLocalizedString(@"扫一扫", "");
    
    // 3> 左右按钮
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"关闭", "") style:UIBarButtonItemStylePlain target:self action:@selector(clickCloseButton)];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"相册","") style:UIBarButtonItemStylePlain target:self action:@selector(clickAlbumButton)];
}
